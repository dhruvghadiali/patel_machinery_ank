...
import ClientSectionProjectDialogComponent from "@ScreenComponents/clients/clientSectionProjectDialogComponent";
import _ from "lodash";

const ClientsSectionComponent = ({ clientsData, visibleCards }) => {
  const [projectsByClientCategory, setProjectsByClientCategory] = useState(null);
  const [clientCategory, setClientCategory] = useState(null);
  const [isDialogOpen, setIsDialogOpen] = useState(false);

  const handleCardClick = (client, index) => {
    setClientCategory(client);
    setProjectsByClientCategory(
      projects && projects.length > 0
        ? _.orderBy(
            _.filter(projects, function (project) {
              return _.toLower(project.category) === _.toLower(client.category);
            }),
            ['year'],
            ['desc']
          )
  ...
  return (
    <>
      <ClientSectionProjectDialogComponent
        isDialogOpen={isDialogOpen}
        setIsDialogOpen={setIsDialogOpen}
        projectsByClientCategory={projectsByClientCategory}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 sm:gap-10 md:gap-12">
        {clientsData.map((client, index) => (
          ...
          >
            <div
              className="bg-gray-50 dark:bg-gray-800 hover:cursor-pointer rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-500 p-6 sm:p-8 h-full border border-gray-200 dark:border-gray-700 group-hover:border-orange-300 dark:group-hover:border-orange-600 group-hover:-translate-y-2"
              onClick={() => handleCardClick(client, index)}
            >
              {/* Client Header */}
